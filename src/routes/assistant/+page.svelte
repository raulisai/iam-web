<script lang="ts">
  ...
  type ChatMessageType = Awaited<ReturnType<typeof createChatMessage>>;
  ...
  function handleOffline() {
    ...
    error =
      "Network connection lost. Voice recognition requires an active internet connection.";
        ...
        messages = [
          {
            ...
            role: "system",
            content:
              "Connected to your AI assistant. How can I help you today?",
            timestamp: new Date().toLocaleTimeString("en-US", {
              hour: "2-digit",
              minute: "2-digit",
            }),
...
{#if isLoading}
  <div
    class="flex items-center justify-center h-[calc(100dvh-4rem)] bg-neutral-950"
  >
    <div class="text-center">
      <div
        class="w-12 h-12 border-4 border-purple-500/30 border-t-purple-500 rounded-full animate-spin mx-auto mb-4"
      ></div>
...
{:else}
  <!-- Vista mobile-only -->
  <div
    class="md:hidden h-[calc(100dvh-4rem)] overflow-hidden bg-neutral-950 relative"
  >
    <!-- Sidebar móvil -->
    {#if isSidebarOpen}
      <div class="absolute inset-0 z-50 flex">
        ...
        <!-- Sidebar content -->
        <div
          class="relative w-64 bg-neutral-900 border-r border-white/10 flex flex-col"
        >
          <!-- Header del sidebar -->
            ...
            {#each allSessions as session}
              ...
              >
                ...
                <div
                  ...
                  on:keydown={(e) =>
                    e.key === "Enter" && deleteSession(session.id, e)}
    ...
    <div class="flex flex-col h-full">
      ...
      <div class="px-4 py-3 border-b border-white/10">
        <div class="flex items-center justify-between mb-2">
          ...
          <div class="flex items-center gap-2">
            <span
              class="w-2 h-2 rounded-full {selectedAgent.status === 'online'
                ? 'bg-emerald-400'
                : 'bg-neutral-500'}"
            ></span>
          ...
          <div class="px-4 py-2">
            <div class="flex items-center gap-2 text-white/50">
              ...
              <div class="flex gap-1">
                <span
                  class="w-1.5 h-1.5 rounded-full bg-white/50 animate-bounce"
      ...
      <div class="border-t border-white/10 p-4">
          ...
          <div class="flex items-center gap-2">
            {#if speechSupported}
              <div class="relative group">
                <button
                  ...
                  class="p-2 rounded-lg transition-all {isListening
                    ? 'bg-red-500/20 text-red-400'
                    : 'bg-neutral-800/50 text-white/70 hover:bg-neutral-800'}"
                  aria-label={isListening
                    ? "Stop listening"
                    : "Start voice input (requires internet)"}
                >
                  {#if isListening}
                    <svg
                ...
                <!-- Tooltip -->
                <div
                  class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-neutral-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap"
                >
            ...
            {#if ttsSupported}
              <button
                ...
                class="p-2 rounded-lg transition-all {isSpeaking
        ...
        <div class="flex gap-2 mt-3 overflow-x-auto">
          <button
            on:click={() =>
              handleSuggestedPrompt("Analyze my progress this week")}
          ...
          <button
            on:click={() =>
              handleSuggestedPrompt("Suggest improvements for my goals")}
            class="px-3 py-1 rounded-lg bg-neutral-900/60 border border-white/10 text-[10px] text-white/70 whitespace-nowrap active:scale-95 transition-transform"
          >
  ...
  <div class="hidden md:flex h-[calc(100dvh-4rem)] bg-neutral-950">
    ...
    <div class="w-64 bg-neutral-900 border-r border-white/10 flex flex-col">
      ...
      <div class="flex-1 overflow-y-auto p-3">
        <h3
          class="text-xs font-semibold text-white/40 uppercase tracking-wider mb-2 px-2"
        >
          Recent Chats
        </h3>
        {#each allSessions as session}
      ...
      <div class="px-8 py-6 border-b border-white/10">
        <div class="flex items-center justify-between mb-4">
          ...
          <div class="flex items-center gap-3">
            <span
              class="w-2.5 h-2.5 rounded-full {selectedAgent.status === 'online'
                ? 'bg-emerald-400'
                : 'bg-neutral-500'}"
            ></span>
        ...
        <div class="flex items-center justify-between mb-4">
          ...
          <div class="flex items-center gap-3">
            {#if speechSupported}
              <button
                on:click={toggleListening}
                class="p-2.5 rounded-lg transition-all {isListening
                  ? 'bg-red-500/20 text-red-400'
                  : 'bg-neutral-800/50 text-white/70 hover:bg-neutral-800'}"
                aria-label={isListening
                  ? "Stop listening"
            ...
            {#if ttsSupported}
              <button
                on:click={speakLastMessage}
                class="p-2.5 rounded-lg transition-all {isSpeaking
                  ? 'bg-purple-500/20 text-purple-400'
                  : 'bg-neutral-800/50 text-white/70 hover:bg-neutral-800'}"
                aria-label={isSpeaking ? "Stop speaking" : "Speak last message"}
